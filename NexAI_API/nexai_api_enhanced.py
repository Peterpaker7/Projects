...
try:
    ...
    TWILIO_PHONE_NUMBER = os.getenv('TWILIO_PHONE', '7904336751')  # Your Twilio number
...
PATIENTS_DB = {
    "kavitha": {
        ...
        "doctor_phone": "+917904336751",
        "family_phone": "+917904336751",
        "email": "testingpurpose905@gmail.com",
        "doctor_email": "testingpurpose905@gmail.com",
        "last_vitals": None,
...
def get_shap_explanation(input_data):
    ...
    try:
        ...
        for i, feature in enumerate(feature_names):
            val=shap_for_prediction[i]
            if hasattr(val,'item'):
                val=val.item()
            feature_importance[feature]=abs(float(val))
...
def predict():
    ...
    try:
        ...
        response = {
            ...
            'probabilities': {
                'high_risk': round(float(prediction_proba[0] * 100), 1),
                'low_risk': round(float(prediction_proba[1] * 100), 1),
                'mid_risk': round(float(prediction_proba[2] * 100), 1)
            },
            'subtags': subtags,
...
@app.route('/send-alert/<patient_id>', methods=['POST'])
def send_patient_alert(patient_id):
    """Send custom alert to patient - Used by clinician dashboard"""
    try:
        patient = PATIENTS_DB.get(patient_id.lower())
        if not patient:
            return jsonify({'error': 'Patient not found'}), 404
        ...
        data = request.get_json()
        message = data.get('message', 'Please check in with your doctor.')
        urgent = data.get('urgent', False)
        
        # Send SMS
        if urgent:
            sms_text = f"⚠️ URGENT from doctor:\n\n{message}\n\nContact clinic immediately. - NexAI"
        else:
            sms_text = f"📨 From doctor:\n\n{message}\n\n- NexAI"
        
        sms_sent = send_sms(patient['phone'], sms_text)
        
        # Send email
        email_subject = "⚠️ URGENT: Message from Your Doctor" if urgent else "Message from Your Doctor"
        email_body = f"""
        <html>
        <body style="font-family: Arial;">
            <div style="background: #f5f5f5; padding: 20px;">
                <div style="background: white; padding: 30px; border-radius: 10px; max-width: 600px; margin: 0 auto;">
                    <h2 style="color: {'#e53e3e' if urgent else '#00b4d8'};">
                        {'⚠️ URGENT MESSAGE' if urgent else '📨 Message from Doctor'}
                    </h2>
                    <p style="font-size: 16px; color: #333;">{message}</p>
                    {'<p style="color: #e53e3e; font-weight: bold;">Contact clinic immediately.</p>' if urgent else ''}
        ...
        return jsonify({
            'success': True,
            'message': f'Alert sent to {patient["name"]}',
            'notifications_sent': {
                'sms': sms_sent or TWILIO_ENABLED,
                'email': email_sent or EMAIL_ENABLED
            }
        })
